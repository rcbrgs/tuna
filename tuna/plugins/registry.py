"""This module's scope are the operation related to Tuna's plugin system's
registry.

In very brief terms, a function may be registered as a Tuna plugin as long as it
only uses keyworded arguments, and if it replaces an existing plugin, it must
have the same number, name and type of arguments.

Tuna's pipelines, instead of calling the tools by its module name, call the
module that is registered as the implementation for a given "tool". For example:
the high resolution data reduction pipeline needs to call *some* tool that
detects noise in a raw data cube. Instead of calling directly the module
tools/noise.py, it calls the module registered under the "Noise detector" entry
in the registry.
...
In this output, notice the annotations regarding the type of each parameters, and
the annotation regarding the return value. This annotation format is compatible
with Python's `type hint system <https://docs.python.org/3.5/library/typing.html>`_.
In very brief terms, after each variable name, a colon ( ":" ) is added, and the
type of that variable is specified. If there is a default value, that is
specified after the type. After the closing parenthesis, an arrow ( "->" ) is
added, and after that, the type of the return value.

If some value (for instance, the return value) is a tuple of several types, it
must be fully described, like so::

  def function ( variable: type ) -> ( int, float, str, list )
    
The pipelines in Tuna are recipes that step through the workflow by calling the
module associated with the corresponding step, in the registry. This call is made
like this::
  ...
  noise = tuna.plugins.run("Noise detector") (
      noise_mask_radius = 1, 
      noise_threshold = 1, 
      raw = raw_can, 
      wrapped = barycenter_can)

  # (We now have the noise array loaded in the "noise" variable).
    
Now let's suppose you need a different noise calculation - for instance, the
current one does not work in your dataset. However, you do not want to re-write
the pipeline for high-resolution data reduction. What you want is to change the
current noise calculation.

What you need to do is:

#. Create a function with the same signature as defined for that plugin type, on
the registry.

#. Register this new function in the registry, under the name specified as the
plugin type.
...
Some remarks: the numpy.copy function is not a requirement; as long as the return
value of the function obeys the signature, it will be accepted as a valid plugin.
Notice that in the worked-out example, the function signature does not change, as
expected.
At the end of this plugin setup, any pipeline can be called, and they will then
use the newly set noise detector.
It is possible to "break" the pipeline, for instance the shape and axis order of
the numpy arrays is not checked at all, and it is expected that the plugins will
do "the right thing". In this example, the plugin will receive a 3D numpy array,
and return a 2D numpy array, with the same number of columns and rows as the
input.

Finally, if you want to add a plugin that has no signature yet, the syntax is
identical to the one to modify a plugin:
...
def get_full_module_path(reference: object) -> str:
    """Return a standardized string version of the full module path and function
    name, when given a reference to a function as input.
...
def registry(step_name: str = "",
    ...
    """User interface to the registry.

    If called with no step_name parameter, it will print the current registry to
    stdout.

    If called with only the step_name parameter, it will check that argument is a
    key in the registry's dictionary, and print the signature associated with
    that key if so.

    If called with two parameters, it will attempt to add or modify the
    dictionary so that the second parameter is the entry with the key contained
    ...
    * callable_reference : object : None
    ...
    if step_name == "":
        ...
        for key in sorted(list(__registry.keys())):
            print("* \"{}\": {}".format(
                key, get_full_module_path(__registry[key])))
        return
    ...
    if step_name not in list(__registry.keys()):
        ...
        print("Added {} to the plugin registry.".format(
    ...
    for key in current_annotation.keys():
        try:
            ...
            print("Aborted registry change, new plugin signature differs from " \
                  "previous one for \"{}\".".format(key))
            return
...
def run(step_name: str) -> object:
    """Return a reference to the callable object associated with the registry key given as input.
    """
    try:
