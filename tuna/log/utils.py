...
__changelog = {
    ...
    "0.1.0": {"Tuna": "0.16.0", "Change": "Fixed setting verbosity on the " \
              + "console would also set it on the file. Added defaults to " \
              + "verbose() parameters, and an 'all' option to the handler_type"\
              + " parameter." }
}
...
def function_header():
    """This method's goal is to log function entry-point capturing relevant 
    debug information.

    No parameter is needed, since all relevant information is obtained through 
    ...
    if arguments != 0:
        for variable_number in range(arguments):
            ...
            variables_string += str(
                sys._getframe().f_back.f_locals[variable_name])
...
def function_name ( ):
    """Access the name of the caller function.
...
def line_number():
    """Provide the line number from which the caller called this function, for 
    logging purposes.

    Returns:

    * unnamed variable : int
        Containing the line number (in the relevant source file) from which the
        caller called.
    """
    return sys._getframe().f_back.f_lineno

def script_name():
    """Access the name of the script currently being processed.
...
def set_path(file_name):
    """Set the path and file name where the log output will be saved.

    Parameters:

    * file_name : string
        Containing a valid path and file name for a text file, where new log 
...
def verbose(handler_type = "all", verbosity = "WARNING"):
    """Set the specified logging handler type to the specified verbosity.
    ...
    * verbosity : string : "WARNING"
        Must be the name of a level from the logging module, such as "DEBUG" or
        "INFO".
    """
    try:
