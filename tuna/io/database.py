...
__changelog__ = {
    ...
    "0.1.1": {"Tuna": "0.16.0", "Change": "Clean up of changelogs / versions" \
              + "on individual classes/methods. Made message about lack of" \
              + "MySQL a debug message." },
    "0.1.0": {"Tuna": "0.15.3", "Change": "Made message about lack of SQL" \
              + "server during select less spammy." }
    }
...
class Database(threading.Thread):
    """This class' responsibilities are creating and maintaining a connection to
    a system's MySQL database. It is also the gateway through which queries are 
    to be made.
    ...
    This module is part of the "guts" of Tuna and is not meant as a 
    user-serviceable module.
    """
    def __init__(self):
    ...
    def run(self):
        """This method is required by :ref:`threading_label`, which allows
        parallel exection in a separate thread.

        This method's goal is to connect to a MySQL daemon, verify that it has 
        the appropriate tables and keep the connection open until the object is
        stopped.
        """
        ...
        if not self.check_mysql_connection():
            ...
            # Check again.
            if not self.check_mysql_connection():
                self.log.debug("Could not open MySQL connection, aborting use" \
                               + " of database.")
                return
    ...
    def stop(self):
        """Initiate a shutdown sequence of the object, which will disconnect 
        from the MySQL daemon and stop the thread.
        """
        self.shutdown = True
        self.close_mysql_connection()
    ...
    def check_mysql_connection(self):
        """Verify that the connection to the database manager is working.

        Returns:
        
        unnamed variable : bool
            This value corresponds to the self.connection object being None or 
            existing.
        """
    ...
    def open_mysql_connection(self):
        """Connect to the database manager. Which should reside on the 
        'localhost', have a database 'tuna' and a user 'tuna' that has all 
        rights on the database 'tuna', using the password 'tuna'.
        """
        try:
            ...
            self.log.debug("Exception during MySQL connection open: {}.".format(
                e))
            self.connection = None
    ...
    def check_tables(self):
        """Verify that the database connected to contains the proper tables for
        running Tuna.
        """
        try:
        ...
        if sorted(result_tables)!=sorted(expected_tables):
            self.log.debug("Tables in db '{}' differ from the " \
                           + "expected '{}'!".format (
                               result, expected_tables))
            return False
        return True

    def configure_tables(self):
        """Populate an empty database with a structure defined in 
        self.expected_tables.
        """
        try:
            cursor = self.connection.cursor()
        ...
        if result != ():
            self.log.error("Database '{}' is not empty AND is different from" \
        ...
        for table in self.expected_tables.keys():
            try:
                ...
                cursor.execute("create table {} {}".format(
                    table, self.expected_tables[table]))
                self.connection.commit()
                cursor.close()
            except Exception as e:
                self.log.error(tuna.console.output_exception(e))
    ...
    def insert_record(self, table, columns_values):
        """
        This method's goal is to enqueue a request to 
        self.insert_record_processor ( ).
        ...
        * columns_values : dictionary
            A dictionary where keys must contain valid column identifiers for 
            the specified table, and the dictionary values must be valid values
            of the type the database specifies for that column, in that table.
        """
    ...
    def insert_record_processor(self, table, columns_values):
        """Process a insert statement in the database manager. 
    ...
    def select_record( self, table, columns_values):
        """Process a select statement in the database manager.
        ...
        * unnamed variable : bool
            Will be True if the connection is open, and data was retrieved 
            without raising an exception. Otherwise, will return False.
        """
        for key in columns_values.keys():
            try:
    ...
    def update_record(self, table, columns_values):
        """Enqueue a request to update_record_processor ( ).
    ...
    def update_record_processor(self, table, columns_values):
        """Process an update statement in the database manager.
    ...
    def enqueue(self, data):
        ...
        * data : dictionary
            Must contain valid entries for the following keys: function, args 
            and kwargs.
        """
