...
__changelog = {
    "0.2.1": {"Tuna": "0.16.5", "Change": "fixed syntax for ndarrays creation,"\
              + " avoiding floats in shape."},
    "0.2.0": {"Tuna": "0.14.0", "Change": "improved docstrings."},
    "0.1.1": {"Change": "Updated docstrings to new style documentation."},
...
class Ada(FileReader):
    ...
    * array : numpy.ndarray : defaults to None
        An array containing the image data; this parameter is useful to convert 
        a known array to a photon table.

    * file_name : string : defaults to None
        To obtain data from a file, it must contain the path to a valid ADT 
        file, which will contain the metadata and the names for the individual 
        ADA files.

    Example usage::

        import tuna
        raw = tuna.io.ada ( file_name = "tuna/tuna/test/unit/unit_io/G093/" \
            + "G093.ADT" )
        raw.read ( )
        raw.get_array ( )
        raw.get_metadata ( )
    ...
    def _read_adt(self):
        ...
        for element in range(len(photon_files)):
            ...
            if last_printed < percentage_done:
                self.log.info("Adding photon counts into numpy array: %3d" % (
                    percentage_done * 10) + '%')
    ...
    def _read_ada(self, channel = -1, file_name = None):
        ...
        * channel : integer : defaults to -1
            An ADA file will often encode the photon counts for a given channel.
            This channel number must be specified so that the photon table can
            be constructed with this information. If the default value of -1 is
            used, the method will return immediately.
    ...
    def _read_adt_metadata(self):
        """Extract metadata from an ADT file.
        """
        self.log.debug(tuna.log.function_header())
