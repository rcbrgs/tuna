    ...
    * array : numpy.ndarray : defaults to None.
    ...
    def _discover_adhoc_type ( self ):
        ...
        if self.__file_name:
            ...
            print("self.__file_object.tell=",self.__file_object.tell())

            self.__file_object.close ( )

            #----new  add by Julien Penguen 28/09/2017----#
    ...
    def get_element_array ( self, x=None, y=None, z=None ):
        ...
        This method's goal is to return the current value of the element in data array.
        ...
        try:
            ...
            if self.__adhoc_type == 2:
                ...
                if (type(x) is not int) or (type(y) is not int):
                    raise TypeError
                if (x >= (self.get_array().shape[1])) or (y >= (self.get_array().shape[0])):
                    raise IndexError

                if (x <= ((self.get_array().shape[1])-1)) and (y <= ((self.get_array().shape[0])-1)):
            ...
            #-----test fichier ad3-----#
                ...
                if (x > ((self.get_array().shape[2])-1)) or (y > ((self.get_array().shape[1])-1)) or (z > ((self.get_array().shape[0])-1)):
                    raise IndexError
                if (x <= ((self.get_array().shape[2])-1)) and (y <= ((self.get_array().shape[1])-1)) and (z <= ((self.get_array().shape[0])-1)):
    ...
    def read ( self,facteur_xyz=True ):
        ...
        self.__file_object = open ( self.__file_name, "rb" )# new add by Julien Penguen 28/09/2017

        print("self.__array_size=",self.__array_size)
        ...
        if self.__adhoc_type == 3 or self.__adhoc_type == -3:
            ...
            #--new add by Julien Penguen 17/07/2017--#
            if facteur_xyz ==True:
                self._read_adhoc_3d ( )
            else:
    ...
    def _read_adhoc_2d ( self ):
        ...
        adhoc_2d_file_type = numpy.dtype ( [ ( 'data', np.float32, int(self.__array_size)),
                                         ( 'trailer',
                                               [ ( 'nbdim', np.int32 ),
                                                 ...
                                                 ( 'none2', np.int8, 1 ),
                                                 ( 'none', np.int32, 4 ),
                                                 ( 'comment', np.int8, 128 ) ] ) ] ) #modified by Julien Penguen 13/09/2017
        ...
        if ( numpy_data['trailer']['lx'] >= 32768 ) |  ( numpy_data['trailer']['ly'] >= 32768 ):
            ...
            self.log.info ( 'critical: If you want to allow arrays as large as this, modify the code!' )
            return

        try:
            self.__array = numpy_data['data'][0].reshape ( int(numpy_data['trailer']['ly']),
                                                           int(numpy_data['trailer']['lx']) ) #modified by Julien Penguen 13/09/2017
        except ValueError as e:
        ...
        #------------------------------------#
        #new add by Julien Penguen 12/09/2017
    ...
    def _read_adhoc_3d ( self, xyz = True ):
        ...
        * xyz : boolean : defaults to True.
        ...
        data.seek ( 0, 2 )
        sz = ( data.tell ( ) - 256 ) / 4
        sz = int(sz) #new add by Julien Penguen 12/07/2017
        ...
        if ad3['trailer']['nbdim'] == -3:  # nbdim ?
            data = ad3['data'][0].reshape(int(ad3['trailer']['lz']), int(ad3['trailer']['ly']), int(ad3['trailer']['lx']))  #new add by Julien Penguen 3/10/2017
        else:
            data = ad3['data'][0].reshape(int(ad3['trailer']['ly']), int(ad3['trailer']['lx']), int(ad3['trailer']['lz'])) #new add by Julien Penguen 17/07/2017

        if xyz & (ad3['trailer']['nbdim'] == 3):
            #return the data ordered in z, y, x
            data = data.transpose(2, 0, 1)

        print("ad3['trailer']['nbdim'] =",ad3['trailer']['nbdim'])
        print('xyz=',xyz)
        ...
        self.__array = data
        #self.__adhoc_trailer = data['trailer'] #new add by Julien Penguen 25/09/2017
        #self.__array [ np.where ( data == -3.1E38 ) ] = np.nan
        self.__adhoc_list_element = dt['trailer'].names #new add by Julien Penguen 20/07/2017
        self.log.info ( "Successfully read adhoc 3d object from file %s." % str ( self.__file_name ) )

        #------------------------------------#
        #new add by Julien Penguen 12/09/2017

        self.__adhoc_trailer=dict(zip(dt['trailer'].names,ad3['trailer'][0])) #new add by Julien Penguen 11/09/2017
    ...
    def get_photons ( self ):
        ...
        * self.__photons : dictionary
        ...
        """
        return self.__photons #new add by Julien Penguen 12/09/2017
    ...
    def get_trailer ( self ):
        ...
        * self.__trailer : numpy.ndarray
        ...
        """
        return self.__adhoc_trailer #new add by Julien Penguen 12/07/2017

    def get_element_trailer ( self, param ):
        """
        New function add by Julien Penguen  20/07/2017

        This method's goal is to return the value of a current trailer parameter.
