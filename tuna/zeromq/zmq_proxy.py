...
class ZMQProxy ( ):
    ...
    def __call_log(self, msg):
        """Dispatch the input message to a log server.
        ...
        * msg : string
        ...
        if answer.decode("utf-8") != 'ACK':
            ...
            print(u'Received: "%s" from %s.' % answer.decode("utf-8"),
                  msg_destination)
    ...
    def __call_print(self, msg):
        """Print a received message. It is meant as a fallback in case the log
    ...
    def check_ACK(self, ack_msg):
        """Verify that the input ZeroMQ message contains the string "ACK".
    ...
    def close(self):
        """Gracefully shutdown the ZeroMQ proxy.
        """
    ...
    def run(self):
        ...
        It  will run in loop, listening to messages and dispatching them as
        appropriated.
        
        Note to developers: destination_call_table is a dictionary associating
        target strings with the functions to be run. The services responsible for
        a given target can be changed here without changing the clients.
        """
        ...
        while self.__lock == True:
            ...
            if self.__zmq_socket_rep in zmq_buffer and \
               zmq_buffer[self.__zmq_socket_rep] == zmq.POLLIN:
