...
__changelog = {
    ...
    "0.1.0": {"Tuna": "0.15.0", "Change": "Added changelog. Moved to " \
            "tuna.tools.continuum. Refactored as a plugin."}
}
...
class Detector(threading.Thread):
    """This class is responsible for detecting the continuum at each pixel, for a
    given input data cube.
    ...
    def run(self):
        ...
        for row in range(self.can.array.shape[1]):
            ...
            for col in range(self.can.array.shape[2]):
                continuum_array[row][col] = median_of_lowest_channels(
                    spectrum = self.can.array[:, row, col], 
...
def median_of_lowest_channels(continuum_to_FSR_ratio = 0.25,
    ...
    """Obtain the median of the three lowest channels of the input profile.
...
def suppress_channel(replacement,
                     array = numpy.ndarray,
                     channels = list):
    """Create a copy of the input array, substituting the input channels list
    with the channels from the input replacement.
...
def continuum_detector(raw: tuna.io.Can,
    ...
    """Conveniently return a Tuna can containing the continuum data for the given
    input.
    ...
    * continuum_to_FSR_ratio : float
    ...
    continuum_detector_object = detector(
        can = raw,
