...
__changelog = {
    ...
    "0.1.0": {"Tuna": "0.14.0", "Change": "improved documentation, silenced logs."}
}
...
class ArcSegmentationCenterFinder(threading.Thread):
    """This class' responsibility is to find the center of an image by finding
    the intersection of rays computed from arc segments.
    ...
    * wrapped : can
        Containing a wrapped phase map, such as one generated by one of the
        barycenter's tools in Tuna.
    ...
    Example:
        ...
        >>> center = tuna.tools.phase_map.arc_segmentation_center_finder(
                noise = noise, wrapped = barycenter); center.join()
        >>> center.get_center()
        (218, 254)
    ...
    def run(self):
        """Execute the main algorithm once its  thread .start() method is called.
        """
        start = time.time()
        ...
        self.log.debug("find_image_center_by_arc_segmentation() took %ds." % (
            time.time() - start))

    def detect_ring_borders(self):
        """Detect borders, and select only the first connected set of pixels in
        this border.
        """
        start = time.time()

        ring_borders_map = numpy.zeros(
            shape = self.wrapped.shape, dtype = numpy.int16)
        ...
        for row in range(max_rows):
            for col in range(max_cols):
                neighbours = tuna.tools.get_pixel_neighbours(
                    (row, col), ring_borders_map)
                distances = []
                for neighbour in neighbours:
                    distances.append(int(abs(
                        self.wrapped.array[row][col] - \
        ...
        if max_arc_count < 20:
            self.log.error("max_arc_count = %d, which is too small. " \
                           "Assuming no border." % max_arc_count)
            self.__ring_borders = numpy.ones(shape = ring_borders_map.shape)
            return

        self.__ring_borders = numpy.copy(ring_borders_map)

        self.log.debug("detect_ring_borders() took %ds." % (
            time.time() - start))

    def find_center(self):
        """Find the center by segmenting arcs and searching for the intersection
        of rays perpendicular to these segments.
        """
        ...
        while (True):
            if (convergence_tries > 50):
                self.log.warning("Reached threshold for center convergence " \
                                 "using chord perpendiculars.")
                break
            ...
            self.log.debug("o_chord_bisector_0 = %s, slope = %s" % (
                str(chord_bisector_0.equation()),
                str(float(chord_bisector_0.slope.evalf()))))
            self.log.debug("o_chord_bisector_1 = %s, slope = %s" % (
                str(chord_bisector_1.equation()),
                str(float(chord_bisector_1.slope.evalf()))))
            bisections.append((chord_bisector_0, chord_bisector_1,
                               convergence_tries * 10))

            # sometimes intesection fails because one of the bisectors ain't a
            #GeometryEntity.
            try:
                intersection = sympy.geometry.intersection(
                    chord_bisector_0, chord_bisector_1)
            except ValueError as e:
                self.log.warning("ValueError %s. Ignoring this intersection." % (
                    e))
                convergence_tries += 1
                continue
            ...
            if (len(intersection) != 0):
                ...
                self.log.debug("o_point_center = %s" % str(
                    (intersect_row, intersect_col)))
                self.log.debug("i_convergence_tries * 10 = %s" % str(
                    convergence_tries * 10))
    ...
    def get_center(self):
        """Access the calculated center coordinates. Will trigger a find if the
        center is yet unknown.
    ...
    def get_most_distant_points(self, tl_points = []):
        """Find the pair of points that are most distant in a list of points, by
        calculating the distance between each possible pair.
        ...
        * unnamed variable : list
            Containing the first two points that are separated by the largest
            distance.
        """
        max_distance = 0
        max_origin = 0
    ...
    def get_random_chord_bisector(self):
        """Find a perpendicular bisector for line segments determined from two
        random points from the current border.
        ...
        * unnamed variable : sympy.Line
        ...
        point_0 = sympy.Point(max_distance_points[0][0],
                              max_distance_points[0][1])
        point_1 = sympy.Point(max_distance_points[1][0],
                              max_distance_points[1][1])
        self.log.debug("debug: point_0, point_1 = %s, %s" % (str(point_0),
                                                             str(point_1)))
    ...
    def get_random_point_in_border(self):
        """Access a random point from the current border.
        
        Returns:
        
        * unnamed variable : tuple of 2 integers
            Corresponding to the column and row of a random position in the
            border.
        """
