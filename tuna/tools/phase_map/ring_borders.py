# -*- coding: utf-8 -*-
"""This module's scope is the detection and mapping of borders in a wrapped phase
map.
...
class RingBorderDetector(threading.Thread):
    """This class is responsible for detecting the "borders" of the rings
    contained in a interferogram. 
    ...
    * rings : dictionary
        Such as the one produced by :ref:`tuna_tools_spectral_rings_fitter_label`
        or equivalent.
    ...
    * log_level : valid :ref:`logging_label` level : logging.INFO
    ...
    def run(self):
        ...
        First, this tool will aggregate all circles contained in the input rings
        dictionary. Then, it will map the distances from each pixel in the
        borders to the center of the rings structure. This is saved as the
    ...
    def generate_borders_map_from_wrapped(self):
        """The directional gradients from the wrapped phase map will have low
        values everywhere, except at the transition from 0 to max channel,
        indicative of an order change.
        This method will detect that and mark the border pixels with a one, and
    ...
    def map_distances_onto_borders(self):
        """Map the pixels belonging to the border with the value of the radius of
        its nearest ring. Essentially, the objective is to have every pixel that
        belongs to a certain "border" to have the same non-zero value, and for
        different ring borders to have different values.
        """
