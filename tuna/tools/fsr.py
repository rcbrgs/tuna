# -*- coding: utf-8 -*-
"""Module fsr is responsible for computing the relative number of FSRs from the
axis until each pixel.
...
Example usage::
    ...
    >>> rings = tuna.plugins.run( "Ring center finder")(data = raw.array, \
                                                        min_rings = 2)
    >>> borders = tuna.tools.phase_map.ring_border_detector(
            barycenter, (219, 255), noise, rings); borders.join()
    >>> fsr = tuna.plugins.run("FSR mapper")( \
            distances = borders.distances, \
            wrapped = barycenter, \
            center = (219, 255), \
            concentric_rings = rings['concentric_rings'])
    >>> fsr.array[0][150 : 200]
...
class FSR(threading.Thread):
    """Responsible for computing and storing a 2D array of integers with the
    number of FSRs from the axis until each pixel.
    ...
    * concentric_rings : dictionary
        A structure describing the geometry of the concentric rings
        characteristic of a Fabry-Pérot interferogram.
    """
    ...
    def create_fsr_from_radii(self):
        """Supposing the borders map and the concentric rings center are
        available, this method will generate a list of typical distances from
        border to center, and create a order map using this list. 
        """
        center = self.concentric_rings[0]
        self.log.debug("self.distances.shape == {}".format(
        ...
        while (len(filtered_distances) < target_number):
            ...
            self.log.debug("threshold == {}, filtered_distances == {}".format(
                threshold, filtered_distances))
            ...
            for row in range(rows):
                ...
                for entry in range(len(sorted_distances)):
                    if entry == 0:
                        ...
                        low_limit = sorted_distances[entry] \
                                    - (sorted_distances[entry] \
                                       - sorted_distances[entry - 1]) / 4
                    if entry == len(sorted_distances) - 1:
                        high_limit = sorted_distances[entry] * 1.1
                    else:
                        high_limit = sorted_distances[entry] \
                                     + (sorted_distances[entry + 1] \
                                        - sorted_distances[entry]) / 4
...
def fsr_mapper(distances: tuna.io.Can,
    ...
    """Conveniently compute a 2D array of integers with the number of FSRs from
    the axis until each pixel.
