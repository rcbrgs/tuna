...
def fit_airy ( discontinuum,
    ...
    for dep in range ( discontinuum.shape [ 0 ] ):
        ...
        #beam_min = numpy.amin ( data )
        beam_max = numpy.amax ( data )
        airy_custom_model.beam.min = 0.9 * beam_max
        airy_custom_model.beam.max = 1.1 * beam_max
        ...
        gap_diff = airy_model_fit.parameters [ 5 ] - previous_gap
        log.info ( "gap_diff = %f" % gap_diff )
        previous_gap = airy_model_fit.parameters [ 5 ]

        result [ dep ] = airy_model_fit ( x, y )

        msg = "plane %d, beam = %.1f, finesse = %.1f, focal_length = %.3f, gap = %.6f"
        log.info ( msg % ( dep, 
